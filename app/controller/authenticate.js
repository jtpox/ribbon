 ...
 class Index {
    ...
    signin(req, res)
    {
        if( req.body.email && req.body.password )
        {
            ...
            User.findByEmail(req.body.email, function(err, results) {
                //console.log(results);

                if( results.length > 0 )
                {
                    Bcrypt.compare(req.body.password, results[0].password, (err, check) => {
                        if( check )
                        {
        ...
        if( req.body.session_id && req.body.session_token )
        {
            Session.findBySessionId(req.body.session_id, (err, results) => {
                if( results.length > 0 )
                {
                    ...
                    Bcrypt.compare(req.body.session_token, results[0].token, (err, check) => {
                        if( check )
                        {
                            /*
